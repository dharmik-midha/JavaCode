...
public class FileReadFunctions {
	public static void main(String arg[]) {
		try {
			...
			for (int i = 0; i < 5; i++) {
				System.out.println("In byte form : " + fis.read());// print bytes
				// first from of read
			}
			for (int i = 0; i < 5; i++) {
			...
			byte b1[] = new byte[20];
			{
				...
				System.out.println("read next byte(using third form)  : " + s1);
//third form of read (byte b[],int offset, int n bytes)
			}
